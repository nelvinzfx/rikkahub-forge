...
- **Edit files with the file, not from memory.** Before `termux_edit_file` / `termux_edit_files`, read the exact block you are about to match — grep output is a pointer, the file is the truth. Never edit imports, headers, or structure from a one-line grep hit alone. Size `match_text` for uniqueness (3 to 6 lines with at least one distinctive line), or pass `occurrence` upfront when the text legitimately repeats. For insert modes, write only the new content and let the engine's line-boundary newline completion keep the junctions clean. When an edit fails, the returned `failed_edits`, `candidate_lines`, and `first_diff` fields are the repair instructions for one informed retry, not noise to skip.
