...
internal fun parseCapturedOutputEnvelope(text: String, expectedJobId: String): ProtocolParseResult<CapturedOutputEnvelope> {
    ...
    val fields = setOf(
        ...
        "stdout_head_bytes", "stderr_head_bytes", "stdout_head_b64", "stderr_head_b64",
    )
    val parsed = parseProtocol(text, "RIKKAHUB_JOB_V1", fields)
    ...
    val status = values.getValue("status").takeIf { it in setOf("completed", "timed_out", "cancelled") }
        ?: return ProtocolParseResult.Error("invalid_protocol")
...
internal val JOB_RETENTION_LOCKED_SCRIPT = """
    ...
    for job_dir in "${'$'}jobs_root"/*; do
            ...
            if [ "${'$'}mode" = group ] && [ -z "${'$'}extra" ] && [[ ${'$'}pid =~ ^[1-9][0-9]*${'$'} ]] && [[ ${'$'}root_start =~ ^[1-9][0-9]*${'$'} ]]; then
                ...
                if [ "${'$'}live_pgrp" = "${'$'}pid" ] && [ "${'$'}live_start" = "${'$'}root_start" ]; then
                    ...
                    for stat in /proc/[0-9]*/stat; do
                        ...
                        if [[ ${'$'}member_pgrp =~ ^[1-9][0-9]*${'$'} ]] && [[ ${'$'}member_start =~ ^[1-9][0-9]*${'$'} ]] &&
                            [ "${'$'}member_pgrp" = "${'$'}pid" ] && [ "${'$'}member_start" -ge "${'$'}root_start" ]; then live=true; break; fi
                    done
                fi
            fi
        fi
        ...
        if [ -f "${'$'}job_dir/meta" ]; then
            ...
            case "${'$'}status" in completed|timed_out|cancelled) ;; *) status= ;; esac
...
internal val SPOOL_CAPTURE_SCRIPT = """
    ...
    retention_script=${'$'}6; leader=${'$'}7; managed=${'$'}8
    shift 8
    ...
    for f in stdout stderr meta state stop_reason; do [ -L "${'$'}job_dir/${'$'}f" ] && exit 125; done
    ...
    if [ "${'$'}tombstone" = true ]; then
        ...
        command -v setsid >/dev/null 2>&1 || { printf '%s\n' 'managed capture requires setsid (install util-linux)' > "${'$'}stderr_file"; exit_code=125; }
        if [ "${'$'}exit_code" = 0 ]; then
            setsid bash -c "${'$'}leader" rikka-spool-leader "${'$'}state_file" "${'$'}stop_file" "${'$'}lock_file" "${'$'}@" > "${'$'}stdout_file" 2> "${'$'}stderr_file" &
            leader_pid=${'$'}!
            root_start=
            n=0
            while [ ! -s "${'$'}state_file" ] && kill -0 "${'$'}leader_pid" 2>/dev/null && [ "${'$'}n" -lt 100 ]; do sleep 0.01; n=${'$'}((n + 1)); done
            if [ -s "${'$'}state_file" ] && [ ! -L "${'$'}state_file" ]; then
                read -r state_mode state_pid root_start state_extra < "${'$'}state_file"
                [ "${'$'}state_mode" = group ] && [ "${'$'}state_pid" = "${'$'}leader_pid" ] && [ -z "${'$'}state_extra" ] && [[ ${'$'}root_start =~ ^[1-9][0-9]*${'$'} ]] || root_start=
            fi
            wait "${'$'}leader_pid"; exit_code=${'$'}?
            # A fast root can exit while descendants retain its process group and output fds.
            # Keep state published and wait until every authorized retained member drains. The
            # Android call timeout bounds this loop externally and cleanup can TERM/KILL the group.
            if [ -n "${'$'}root_start" ]; then
                while :; do
                    ...
                    for stat in /proc/[0-9]*/stat; do
                        ...
                        member_pgrp=${'$'}{member_info%% *}; member_start=${'$'}{member_info#* }
                        if [[ ${'$'}member_pgrp =~ ^[1-9][0-9]*${'$'} ]] && [[ ${'$'}member_start =~ ^[1-9][0-9]*${'$'} ]] &&
                            [ "${'$'}member_pgrp" = "${'$'}leader_pid" ] && [ "${'$'}member_start" -ge "${'$'}root_start" ]; then member_found=true; break; fi
                    done
        ...
        # requested for managed=0; execute directly so this wrapper does not claim a kill.
    ...
    printf 'version=1\njob_id=%s\nstatus=%s\nexit_code=%s\ncompleted_at=%s\nstdout_bytes=%s\nstderr_bytes=%s\n' "${'$'}job_id" "${'$'}status" "${'$'}exit_code" "${'$'}completed_at" "${'$'}stdout_total" "${'$'}stderr_total" > "${'$'}meta_tmp" && chmod 600 -- "${'$'}meta_tmp" && mv -f -- "${'$'}meta_tmp" "${'$'}job_dir/meta" || exit 125
    rm -f -- "${'$'}state_file"
    bash -c "${'$'}retention_script" rikka-retention "${'$'}max_jobs" "${'$'}ttl_seconds" "${'$'}job_id" >/dev/null 2>&1
    ...
    flock -u 9
    printf 'RIKKAHUB_JOB_V1\njob_id=%s\nstatus=%s\nexit_code=%s\nstdout_total_bytes=%s\nstderr_total_bytes=%s\nstdout_head_bytes=%s\nstderr_head_bytes=%s\nstdout_head_b64=%s\nstderr_head_b64=%s\n' "${'$'}job_id" "${'$'}status" "${'$'}exit_code" "${'$'}stdout_total" "${'$'}stderr_total" "${'$'}stdout_head_bytes" "${'$'}stderr_head_bytes" "${'$'}stdout_b64" "${'$'}stderr_b64"
    exit "${'$'}exit_code"
""".trimIndent()
...
internal val SPOOL_CLEANUP_SCRIPT = """
    ...
    for f in stdout stderr meta state stop_reason; do [ -L "${'$'}job_dir/${'$'}f" ] && exit 0; done
    unknown=${'$'}(find "${'$'}job_dir" -mindepth 1 -maxdepth 1 ! -name stdout ! -name stderr ! -name meta ! -name state ! -name stop_reason -print -quit)
    [ -z "${'$'}unknown" ] || exit 0
    if [ ! -f "${'$'}job_dir/meta" ]; then
    ...
    if [ -n "${'$'}live_info" ]; then
        ...
        for stat in /proc/[0-9]*/stat; do
            ...
            member_info=${'$'}(awk '{ line=${'$'}0; sub(/^.*\) /, "", line); split(line, f, " "); print f[3], f[20] }' "${'$'}stat" 2>/dev/null)
            member_pgrp=${'$'}{member_info%% *}; member_start=${'$'}{member_info#* }
            if [[ ${'$'}member_pgrp =~ ^[1-9][0-9]*${'$'} ]] && [[ ${'$'}member_start =~ ^[1-9][0-9]*${'$'} ]] && [ "${'$'}member_pgrp" = "${'$'}pid" ] && [ "${'$'}member_start" -ge "${'$'}root_start" ]; then member_found=true; break; fi
        done
...
internal val READ_OUTPUT_SCRIPT = """
    ...
    for f in stdout stderr meta state stop_reason; do [ -L "${'$'}job_dir/${'$'}f" ] && { printf 'RIKKAHUB_READ_V1\nerror=corrupt_job_state\n'; exit 0; }; done
    if [ -f "${'$'}job_dir/meta" ]; then
        ...
        [ "${'$'}meta_version" = 1 ] && [ "${'$'}meta_job" = "${'$'}job_id" ] || { printf 'RIKKAHUB_READ_V1\nerror=corrupt_job_state\n'; exit 0; }
        case "${'$'}meta_status" in completed|timed_out|cancelled) ;; *) printf 'RIKKAHUB_READ_V1\nerror=corrupt_job_state\n'; exit 0 ;; esac
        ...
        live=false
        if [ "${'$'}live_pgrp" = "${'$'}pid" ] && [ "${'$'}live_start" = "${'$'}root_start" ]; then live=true
        elif [ -z "${'$'}live_info" ]; then
            for stat in /proc/[0-9]*/stat; do
                [ -r "${'$'}stat" ] || continue
                member_info=${'$'}(awk '{ line=${'$'}0; sub(/^.*\) /, "", line); split(line, f, " "); print f[3], f[20] }' "${'$'}stat" 2>/dev/null); member_pgrp=${'$'}{member_info%% *}; member_start=${'$'}{member_info#* }
                if [[ ${'$'}member_pgrp =~ ^[1-9][0-9]*${'$'} ]] && [[ ${'$'}member_start =~ ^[1-9][0-9]*${'$'} ]] && [ "${'$'}member_pgrp" = "${'$'}pid" ] && [ "${'$'}member_start" -ge "${'$'}root_start" ]; then live=true; break; fi
            done
        fi
